By default the controller will be exposed under the `/product` URI. This is due to the default `grails-app/conf/mongodb/example/UrlMappings.groovy` class:

[source,groovy]
.grails-app/controllers/mongodb/example/UrlMappings.groovy
----
include::{sourceDir}/grails-app/controllers/mongodb/example/UrlMappings.groovy[tags=defaultMappings,indent=0]
----
