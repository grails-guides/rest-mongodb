...
[source,groovy]
.grails-app/controllers/mongodb/example/ProductController.groovy
----
include::{sourceDir}/grails-app/controllers/mongodb/example/ProductController.groovy[tags=searchAction,indent=0]
----

<1> An action called `search` is defined that takes a query parameter, called `q`, and a `max` parameter
<2> A http://gorm.grails.org/latest/hibernate/manual/index.html#whereQueries[where] query is executed that uses a regular expression (regex) to search for products.
...
With the action written you now need to expose the `/products/search` endoint by defining the appropriate mapping in `grails-app/conf/mongodb/example/UrlMappings.groovy`:

[source,groovy]
.grails-app/controllers/mongodb/example/UrlMappings.groovy
----
include::{sourceDir}/grails-app/controllers/mongodb/example/UrlMappings.groovy[tags=productMapping,indent=0]
----
